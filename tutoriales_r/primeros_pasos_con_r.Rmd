...
# ¿Qué es "R"?   
El programa R es un tipo de software denominado “libre”, que en inglés tiene un doble significado:  
*  No tiene un propietario concreto. Cualquiera puede modificar el código. Así, el funcionamiento del programase enriquece, porque se adapta a las necesidades de los distintos usuarios.  
*	Es gratis y el usuario no tiene que pagar nada por usar el programa.  
...
Si queremos saber qué valores hemos almacenados en *EdadesFamilia*, simplemente escribimos el nombre y le damos a ejecutar.
```{r}
EdadesFamilia
```
Ahora te parecerá que es una orden bastante simplona; pero más adelante nos servirá para saber todos los datos que tenemos en una variable. 
...
Fíjate que la ordenación no afecta a los valores almacenados en *EdadesFamilia*. Si escribes el nombre de la variable, te aparecerán con el mismo (des)orden inicial. Pero si lo que queremos es que los datos queden ordenados permanentemente, los almacenaremos en una nueva variable:
```{r}
EdadesFamiliaorden <- sort(EdadesFamilia)
```

Ahora, si queremos ver el contenido de esa nueva variable que hemos creado, escribimos simplemente el nombre de la variable:  
```{r}
EdadesFamiliaorden
```
