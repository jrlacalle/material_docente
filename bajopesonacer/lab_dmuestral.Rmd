...
```{r m2, eval=FALSE}
set.seed(18080502)
mean(sample(PESON,10, replace=T))
```
...
```{r m4, eval=FALSE}
set.seed(18080502)
mean(sample(PESON,100, replace = T))
```
...
```{r m5, eval=FALSE}
set.seed(18080502)
mean(sample(PESON,1000, replace = T))
```
...
set.seed(18080502)
for (i in 1:10000) {
  maleat5[i] <- mean(sample(PESON,5, replace = TRUE))
}
```
...
set.seed(18080502)
for (i in 1:10000) {
  maleat10[i] <- mean(sample(PESON,10, replace = TRUE))
}
```
...
En primer lugar, consideremos el rango; es decir, la diferencia entre el valor mínimo y el máximo. La orden `range` presenta simultáneamente el valor mínimo y máximo de cada variable. Cuando n=5, estos valores son los siguientes:  

```{r rangos, eval=FALSE}
range(maleat5)
```

Si queremos calcular la amplitud, es decir, la distancia entre los valores máximos y mínimos, usamos una la orden `diff`:  

```{r anchurarango5, eval=FALSE}
diff(range(maleat5))
```

Repite la orden para n=10; en este caso, almacena el rango en una variable que llamarás `maleat10`.

**Pregunta** ¿Qué diferencia encuentras entre la distribución para n=5 y n=10? 

Repite la orden, pero ahora para n=25 y n=100, almacenando los valores en las variables `maleat25`y `maleat100`.  

**Pregunta** Compara el rango cuando n=5 con los valores encontrados cuando n=25 y n=100 ¿Qué diferencia encuentras entre la distribución para n=5 y la de esos dos valores? 
...
<a rel="license" href="http://creativecommons.org/licenses/by/4.0/"><img alt="Licencia de Creative Commons" style="border-width:0" src="https://i.creativecommons.org/l/by/4.0/88x31.png" /></a><br /><span xmlns:dct="http://purl.org/dc/terms/" href="http://purl.org/dc/dcmitype/Text" property="dct:title" rel="dct:type">Distribución del peso al nacer en Andalucía</span> by <span xmlns:cc="http://creativecommons.org/ns#" property="cc:attributionName">Juan Ramón Lacalle Remigio</span> is licensed under a <a rel="license" href="http://creativecommons.org/licenses/by/4.0/">Creative Commons Reconocimiento 4.0 Internacional License</a>.
